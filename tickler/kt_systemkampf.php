...
function wirkungsgrad2($anzahl_jaeger, $anzahl_kreuzer, $anzahl_schlachter, $r, $rtyp){  //große schiffe schlechter machen
	...
	if ($rtyp==1)return($mod_kreuzer);
	elseif ($rtyp==2)return($mod_schlachter);
	else return(1);
}
...
for ($c=0; $c<$z; $c++){
	...
	echo '<br>'.$c.'<br><br>';
	$zsec=$kampfsys[$c][0];$zsys=$kampfsys[$c][1];
  ...
  AND aktion = 1 AND zeit = 1 ORDER BY hsec, hsys";
	...
	while($row = mysql_fetch_array($res)){//f�r jede flotte die daten auslesen
		...
		if ($rsec!=$rsecold OR $rsys!=$rsysold){
			if($db_data["rasse"]==1)$rflag='E';
			elseif($db_data["rasse"]==2)$rflag='I';
			elseif($db_data["rasse"]==3)$rflag='K';
			elseif($db_data["rasse"]==4)$rflag='Z';
			elseif($db_data["rasse"]==5)$rflag='D';

			if ($atterliste!='')$atterliste.=', ';
		...
		if (hasTech($a_userdata[$anz_atter]['techs'],4)) $atter_whg[$anz_atter]=1; else $atter_whg[$anz_atter]=0;
		...
		for($i=0; $i<$sv_anz_schiffe; $i++){
			  ...
			  if ($i==1) $wgrad=wirkungsgrad($row["e82"], $row["e84"], $row["e85"], $row["e88"], $db_data["rasse"]-1);
			  //wirkungsgrad berechnen, wenn es ein kreuzer ist
			  if ($i==3) $wgrad=wirkungsgrad2($row["e81"], $row["e84"], $row["e85"], $db_data["rasse"]-1, 1);
			  //wirkungsgrad berechnen, wenn es ein schlachtschiff ist
			  if ($i==4) $wgrad=wirkungsgrad2($row["e81"], $row["e84"], $row["e85"], $db_data["rasse"]-1, 2);
		...
		echo 'AWEEEEEEEEEEEEEEEEEEEEEEEEEEEEE: '; print_r($atter_awges);
		echo '<br>BWEEEEEEEEEEEEEEEEEEEEEEEEEEEEE: '; print_r($atter_bwges);
  ...
  FROM de_user_fleet where hsec = $zsec AND hsys = $zsys AND (aktion = 0 OR (aktion = 3 AND zeit = 1)) ORDER BY user_id",$db);
	...
	while($row = mysql_fetch_array($res))//f�r jede flotte die daten auslesen
	{
	  ...
	  //schauen ob die whg vorhanden ist
	  if (hasTech($d_userdata[$anz_deffer]['techs'],4)) $deffer_whg[$anz_deffer]=1; else $deffer_whg[$anz_deffer]=0;
  ...
  AND aktion = 2 AND (zeit = 1 OR (zeit = 0 AND aktzeit > 0)) ORDER BY hsec, hsys",$db);
	...
	while($row = mysql_fetch_array($res)){//f�r jede flotte die daten auslesen
		...
		if ($rsec!=$rsecold OR $rsys!=$rsysold) $defferliste.=', '.$db_data["spielername"].' ['.$rflag.']('.$rsec.':'.$rsys.')';
		$rsecold=$rsec;
		$rsysold=$rsys;
	...
	if (hasTech($d_userdata[0]['techs'],6)) $rec=1; else $rec=0;

	//schauen ob es einen planeteren schild gibt
	if (hasTech($d_userdata[0]['techs'],24)) $ps=1; else $ps=0;

	//schauen ob es eine whg gibt
	if (hasTech($d_userdata[0]['techs'],4)) $whg=1; else $whg=0;
	...
	//schauen ob es eine planetare schildwerweiterung gibt und berechne die st�rke
	 if (hasTech($d_userdata[0]['techs'],30)==1){
		...
		//spezialisierung
		if($d_userdata[0]['spec3']==1)$psext+=0.1;

		if($psext<$sv_planetshieldext[0])$psext=$sv_planetshieldext[0];
		if($psext>$sv_planetshieldext[2])$psext=$sv_planetshieldext[2];
	...
	{
	  if(mt_rand(1,100)<=$defense_bonus_feuerkraft[0])$tawartbonus+=$defense_bonus_feuerkraft[1]/100;
	}
	...
	for ($i=0;$i<$anz_atter;$i++){
		...
		for($j=0; $j<$sv_anz_schiffe; $j++){
			...
			if($atter_geb[$i][$j]>$atter[$i][$j])$atter_geb[$i][$j]=$atter[$i][$j];
			if($atter_geb[$i][$j]>$atterrassen[$rasse][$j])$atter_geb[$i][$j]=$atterrassen[$rasse][$j];
			if($atter_geb[$i][$j]>$atterrassen_geblockt[$rasse][$j])$atter_geb[$i][$j]=$atterrassen_geblockt[$rasse][$j];
	...
	for ($i=0;$i<$anz_deffer;$i++){
		...
		for($j=0; $j<$sv_anz_schiffe; $j++){
			...
			if($deffer_geb[$i][$j]>$deffer[$i][$j])$deffer_geb[$i][$j]=$deffer[$i][$j];
			if($deffer_geb[$i][$j]>$defferrassen[$rasse][$j])$deffer_geb[$i][$j]=$defferrassen[$rasse][$j];
			if($deffer_geb[$i][$j]>$defferrassen_geblockt[$rasse][$j])$deffer_geb[$i][$j]=$defferrassen_geblockt[$rasse][$j];
	...
	for($i=$sv_anz_schiffe; $i<$sv_anz_schiffe+$sv_anz_tuerme; $i++)
	{
	  ...
	  if($deffertuerme_geblockt[$i-$sv_anz_schiffe]>$turm[$i-$sv_anz_schiffe])$deffertuerme_geblockt[$i-$sv_anz_schiffe]=$turm[$i-$sv_anz_schiffe];
	...
	for ($i=0;$i<$anz_atter;$i++)
	{
	  ...
	  if($a_userdata[$i]['spec2']==2)$atter_exp[$i]=floor($atter_exp[$i]*1.1);
	}
	...
	for ($i=0;$i<$anz_atter;$i++)
	{
	  ...
	  else $atter_rec[$i]=array(0, 0);//keine whg
	}
	...
	for ($i=0;$i<$anz_deffer;$i++)
	{
	  ...
	  //spezialisierung mehr exp f�r die flotte
	  if($d_userdata[$i]['spec2']==2)$deffer_exp[$i]=floor($deffer_exp[$i]*1.1);
	}
	...
	for($i=$sv_anz_schiffe; $i<$sv_anz_schiffe+$sv_anz_tuerme; $i++){
	  ...
	  if ($ps==1)
	  {
		...
		if($d_userdata[0]['spec3']==1)$ps_bonus+=10;
	...
	if($d_userdata[0]['spec2']==1)$defenseexp=floor($defenseexp*1.5);
	...
	for ($i=0;$i<$anz_deffer;$i++){
		...
		else $deffer_rec[$i]=array(0, 0);//keine whg

		//noch ein test einbauen ob der angegriffene das recyclotron hat, das h�tte in dem fall vorrang
		if ($i<$anz_heimatflotten AND $rec==1) $deffer_rec[$i]=array(0, 0);
	}
...
a:6:{s:5:"daten";a:7:{s:9:"colstolen";i:-1;s:6:"sector";s:1:"2";s:6:"system";s:2:"94";s:6:"rassen";a:5:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:1;i:4;i:4;}s:12:"target_rasse";i:4;s:10:"atterliste";s:17:"Sonntag [Z](13:3)";s:11:"defferliste";s:19:"Ockaol-Ry [D](2:94)";}s:15:"einheiten_atter";a:3:{i:0;a:5:{i:0;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:1;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:2;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:3;a:8:{i:0;i:0;i:1;i:0;i:2;i:58;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:13;i:7;i:0;}i:4;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}}i:1;a:5:{i:0;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}i:1;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;i:0;i:6;i:0;i:7;d:0;}i:2;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}i:3;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}i:4;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}}i:2;a:5:{i:0;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:1;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:2;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:3;a:8:{i:0;d:0;i:1;d:0;i:2;d:4;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:4;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}}}s:16:"einheiten_deffer";a:3:{i:0;a:5:{i:0;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:1;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:2;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:3;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:0;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}i:4;a:8:{i:0;i:0;i:1;i:0;i:2;i:0;i:3;i:8;i:4;i:0;i:5;i:0;i:6;i:0;i:7;i:0;}}i:1;a:5:{i:0;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}i:1;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;i:0;i:6;i:0;i:7;d:0;}i:2;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}i:3;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}i:4;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;i:0;i:7;d:0;}}i:2;a:5:{i:0;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:1;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:2;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:3;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:4;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}}}s:6:"tuerme";a:3:{i:0;a:5:{i:0;s:2:"19";i:1;s:2:"84";i:2;s:3:"331";i:3;s:3:"135";i:4;s:1:"0";}i:1;a:5:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;}i:2;a:5:{i:0;d:10;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;}}s:13:"daten_spieler";a:5:{s:9:"colstolen";s:1:"0";s:3:"exp";d:112;s:9:"kartefakt";d:0;s:10:"recycling1";d:0;s:10:"recycling2";d:0;}s:17:"einheiten_spieler";a:3:{i:0;a:8:{i:0;s:1:"0";i:1;s:1:"0";i:2;s:2:"58";i:3;s:1:"0";i:4;s:1:"0";i:5;s:1:"0";i:6;s:2:"13";i:7;s:1:"0";}i:1;a:8:{i:0;d:0;i:1;d:0;i:2;d:0;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}i:2;a:8:{i:0;d:0;i:1;d:0;i:2;d:4;i:3;d:0;i:4;d:0;i:5;d:0;i:6;d:0;i:7;d:0;}}}
...
*/
	...
	if($zra>=$zrd){
	  ...
	  if($npc==1){
		...
		for ($i=0;$i<$anz_atter;$i++)
		{
		  ...
		  if($anteil>$biggestanteil){$biggestatter=$i;$biggestanteil=$anteil;}  	  
		}
	...
	if ($stolenkollies>0){
		...
		//ally gutschreiben
		if($allyid>0)mysql_query("UPDATE de_allys SET collost = collost + '$stolenkollies' WHERE id='$allyid'",$db);
	}
		...
		//1. feststellen wie viel dem Ziel (nur das Ziel, keine weiteren Deffer an Punkten zerst�rt worden ist
			...
			for ($i=0;$i<$anz_atter;$i++){
				...
				if(($kg[0]>0 OR $kg[1]>0 OR $kg[2]>0 OR $kg[3]>0) && $target_score_lost>0){
					...
					if($getkg[0]>$kg[0]*$anteil){$getkg[0]=$kg[0]*$anteil;}
					if($getkg[1]>$kg[1]*$anteil){$getkg[1]=$kg[1]*$anteil;}
					if($getkg[2]>$kg[2]*$anteil){$getkg[2]=$kg[2]*$anteil;}
					if($getkg[3]>$kg[3]*$anteil){$getkg[3]=$kg[3]*$anteil;}
	...
	for ($r=0;$r<$sv_anz_rassen;$r++){
		for ($s=0;$s<$sv_anz_schiffe;$s++){
		  ...
		  if($wert>$atterrassen[$r][$s])$wert=$atterrassen[$r][$s];
	...
	for ($r=0;$r<$sv_anz_rassen;$r++){
		for ($s=0;$s<$sv_anz_schiffe;$s++){
			...
			if($wert>$defferrassen[$r][$s])$wert=$defferrassen[$r][$s];
	...
	for ($r=0;$r<$sv_anz_rassen;$r++){
	  if ($rassenvorhanden[$r]>1)$rassenvorhanden[$r]=1;
	...
	//echo '<br>ATTER-HP(verloren): '.$vernichtete_atter.' - ATTER-HP(gesamt):'.$atter_allhp.' - ZRA:'.$zra;
	  ...
	  foreach ($global_user_id as $value) {
		  ...
		  $rowg = mysql_fetch_array($db_dateng);
		  $owner_id=$rowg["owner_id"];
		  if ($global_deffer!='')$global_deffer.=';';
	...
	for ($i=0;$i<$anz_atter;$i++){
		...
		if($npc==0)mysql_query("UPDATE de_user_data SET lastpcatt='$rtick' WHERE user_id = '$uid'",$db);
		...
		for($s=0;$s<$sv_anz_schiffe;$s++){
			...
			if($wert>$atter[$i][$s])$wert=$atter[$i][$s];
		...
		if ($atter_sk[$i]>0){
			...
			if($npc==1)$col_angriffsgrenze_final=$sv_min_col_attgrenze;
			...
			if ($col*$col_angriffsgrenze_final<=$kollies){//man erh�lt kollektoren
				...
				if($allyid>0 AND $npc==0)mysql_query("UPDATE de_allys SET colstolen = colstolen + '".$atter_sk[$i]."' WHERE id='$allyid'",$db);
				if($allyid>0 AND $npc==1)mysql_query("UPDATE de_allys SET colstolennpc = colstolennpc + '".$atter_sk[$i]."' WHERE id='$allyid'",$db);
				//allyaufgabe erbeute kollektoren allgemein gutschreiben
				if($allyid>0 AND $npc==0)mysql_query("UPDATE de_allys SET questreach = questreach + '".$atter_sk[$i]."' WHERE id='$allyid' AND questtyp=1",$db);
				if($allyid>0 AND $npc==1)mysql_query("UPDATE de_allys SET questreach = questreach + '".$atter_sk[$i]."' WHERE id='$allyid' AND questtyp=3",$db);
			}else{ //die kollektoren werden vernichtet
				echo 'Kollektoren zerstört.';
				if($allyid>0)mysql_query("UPDATE de_allys SET coldestroy = coldestroy + '".$atter_sk[$i]."' WHERE id='$allyid'",$db);
		...
		*/
		...
		if ($atter_sk[$i]>0 AND $npc==1)mysql_query("UPDATE de_user_data SET npccol = npccol + '$atter_sk[$i]' WHERE user_id = '$uid'",$db);
		//mitloggen wie viele kollektoren man von jemandem holt
		if ($atter_sk[$i]>0 OR $atter_sk[$i]<0){
			$zeit=time();
			if($atter_sk[$i]>0)$colstolen=$atter_sk[$i];else $colstolen=$atter_sk[$i]*(-1);
			if($a_userdata[$i]['spec4']==2)$energiewert=$colstolen*$rtick*15;
			else $energiewert=$colstolen*$rtick*30;
	...
	for ($i=0;$i<$anz_heimatflotten;$i++){
		...
		for ($s=0;$s<$sv_anz_schiffe;$s++){
		  //fix f�r artefakte beim blocken
		  $wert=$deffer_geb[$i][$s];
		  if($wert>$deffer[$i][$s])$wert=$deffer[$i][$s];
		...
		*/
		...
		if($i==$anz_heimatflotten-1){
			...
			//allyaufgabe kriegsartefakte
			if($sv_oscar!=1){
				if($allyid>0 AND $kartefakte>0)mysql_query("UPDATE de_allys SET questreach = questreach + '".$kartefakte."' WHERE id='$allyid' AND questtyp=5",$db);
			}
	...
	for ($i=$anz_heimatflotten;$i<$anz_deffer;$i++){
		...
		*/
		...
		$erges=0;
		for ($s=0;$s<$sv_anz_schiffe;$s++)
		 {$er[$s]=($deffer[$i][$s]-$deffer_zer[$i][$s]); $erges=$erges+($deffer[$i][$s]-$deffer_zer[$i][$s]);}
		$updateid=$d_userdata[$i][0];
		mysql_query("UPDATE de_user_fleet SET
			e81='$er[0]',
			e82='$er[1]',
	...
	if($rec==1){
		...
		if($sv_oscar!=1){
			...
			$rasse=$d_userdata[0][3]-1;
			if($whg==1)$anteil=$sv_recyclotron_bonus_whg; else $anteil=$sv_recyclotron_bonus;
			...
			if($npc==1){
				...
				if($rec_secbonus>$sv_recyclotron_sector_bonus)$rec_secbonus=$sv_recyclotron_sector_bonus;
			...
			if($specrecbonus>10)$specrecbonus=10;
			$anteil+=$specrecbonus;

			//h�chst/minamal-grenze �berpr�fen
			if($anteil<$sv_recyclotron_min)$anteil=$sv_recyclotron_min;
			if($anteil>$sv_recyclotron_max)$anteil=$sv_recyclotron_max;
