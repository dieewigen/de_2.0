...
//�berpr�fen ob es Zeit f�r den Tick ist
...
for ($i=0; $i<$num; $i++){
	...
	if ($tech_id==1000){
			//rasse und geb�udelevel auslesen
...
if ($globtcount==0){
	...
	if ($tronicverteilen>0) echo 'TROOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOONIC';
}
...
//wenn $globzcount null dann schauen ob man die Zufalls-verteilung nochmal anst��t
$zufallverteilen=0;
if ($globzcount==0){
	$r=mt_rand (1, 100);
	//wenn $globzcount auf 1 gesetzt wird, dann beginnt die zufallsverteilung von vorne
	if($r<$sv_globalw_zufall) $zufallverteilen=1;
	else $zufallverteilen=0;
	if ($zufallverteilen>0) echo 'ZUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUFALL';
}
...
for ($rasse=1; $rasse<=$sv_anz_rassen; $rasse++){
	...
	while($irow = mysql_fetch_array($res)){
		...
		$malus=0;$sabotagemalus=0;

		//sabotagemalus
		if($maxtick<$mysc1+$sv_sabotage[7][0] AND $mysc1>$sv_sabotage[7][0])$sabotagemalus+=$sv_sabotage[7][2];
		...
		if (hasTech($pt,20))$emvi=3; else $emvi=6;
		...
		if (!hasTech($pt,14)) $rm=0;
		if (!hasTech($pt,15)) $rd=0;
		if (!hasTech($pt,16)) $ri=0;
		if (!hasTech($pt,17)) $re=0;
		...
		if($spec3cache[$sector]==-1){
		  ...
		  if($planertragbonus>100)$planertragbonus=100;
		...
		if ($tronicverteilen>0){
		  ...
		  if ($tcount==1 AND $npc==0){
			...
			if($npc==0)mysql_query("INSERT INTO de_user_news (user_id, typ, time, text) VALUES ($uid, 8,'$time','$nachricht')",$db);
			$sqlnews=', newnews = 1';
			$tronicertrag+=$tronic;
		...
		if ($zufallverteilen>0 and $tick>$sv_global_start_zufall){
			//niemand hat mehr zuf�lle in der queue, also neu bei jedem verteilen
			$r=mt_rand (4, 240);
			$sqlzcount=', zcount = '.$r;
		}else{
		  //wenn der counter auf 1 ist, dann gibts etwas
		  if ($zcount==1 AND $sector > 1 AND $npc==0){
			...
			$nachricht=$kolliemsg[$zufallsmsg];
			break;
			...
			//schiffe
				...
				$zufallsrasse = $rasse;
				if($anzahl!="1")$zufallsrasse = $rasse+4;

				include $directory."inc/lang/".$sv_server_lang."_wt_zufallmsg.lang.php";
				$nachricht=$schiffmsg[$zufallsmsg];				  
			break;
			...
			//defense
				  ...
				  $zufallsrasse = $rasse;
				  if($zufallsturm!="1")$zufallsrasse = $rasse+4;

				  include $directory."inc/lang/".$sv_server_lang."_wt_zufallmsg.lang.php";
				  $zufallsmsg=mt_rand (0, count($deffmsg)-1);

				  $nachricht=$deffmsg[$zufallsmsg];
			break;
			...
			//Erfahrung
				  ...
				  $nachricht=$erfahrungsmsg[$zufallsmsg];
			...
			//Agenten
					 ...
					 include $directory."inc/lang/".$sv_server_lang."_wt_zufallmsg.lang.php";
					 $nachricht=$agentenmsg[$zufallsmsg];
			...
			//Sonden
					...
					include $directory."inc/lang/".$sv_server_lang."_wt_zufallmsg.lang.php";
					$nachricht=$sondenmsg[$zufallsmsg];
			...
			$nachricht=$ressmsg[$zufallsmsg];
			break;
			}

			if($npc==0) mysql_query("INSERT INTO de_user_news (user_id, typ, time, text) VALUES ($uid, 3,'$time','$nachricht')",$db);
		...
		if     ($col<100) $colscore=10000;
		elseif ($col<150) $colscore=10500;
		elseif ($col<200) $colscore=11000;
		elseif ($col<250) $colscore=11500;
		elseif ($col<300) $colscore=12000;
		elseif ($col<350) $colscore=12500;
		elseif ($col<400) $colscore=13000;
		elseif ($col<450) $colscore=13500;
		elseif ($col<500) $colscore=14000;
		elseif ($col<550) $colscore=14500;
		elseif ($col<600) $colscore=15000;
		elseif ($col<650) $colscore=15500;
		elseif ($col<700) $colscore=16000;
		elseif ($col<750) $colscore=16500;
		elseif ($col<800) $colscore=17000;
		elseif ($col<850) $colscore=17500;
		elseif ($col<900) $colscore=18000;
		elseif ($col<950) $colscore=18500;
		elseif ($col<1000)$colscore=19000;
		else $colscore=20000;
...
if ($globtcount>0) mysql_query("UPDATE de_user_data SET tcount = tcount - 1 WHERE tcount>0",$db);
if ($globzcount>0) mysql_query("UPDATE de_user_data SET zcount = zcount - 1 WHERE zcount>0",$db);
...
if($sv_deactivate_sectorartefacts!=1) include "wt_artefakte.php";
...
echo '<br><br>Plätze und Ränge bestimmen - A: '.strftime("%d/%m/%Y - %H:%M:%S");
...
echo '<br>Plätze und Ränge bestimmen - E: '.strftime("%d/%m/%Y - %H:%M:%S").'<br>';
...
while($irow = mysql_fetch_array($res)){
	...
	$db_daten=mysql_query("SELECT (ac1+ac2+ac3+ac4+ac5+ac6+ac7+ac8+ac9+ac10+ac11+ac12+ac13+ac14+ac15+ac16) AS wert FROM de_user_achievement WHERE user_id='$uid'", $db);
	$num = mysql_num_rows($db_daten);
	if($num==1){
		$row = mysql_fetch_array($db_daten);
