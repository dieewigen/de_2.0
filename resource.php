...
$punkte=$row["score"];$col=$row["col"];$newtrans=$row["newtrans"];$newnews=$row["newnews"];
$sector=$row["sector"];$system=$row["system"];$eartefakt=$row["eartefakt"];
$kartefakt=$row["kartefakt"];$dartefakt=$row["dartefakt"];$palenium=$row["palenium"];
$useefta=$row["useefta"];$mysc1=$row["sc1"];$agent_lost=$row['agent_lost'];
...
$sartefakt=0;$sa_grund = array (0,0,0,0);
...
//werberlink erzeugen
$directory=str_replace("/resource.php","/",$HTTP_SERVER_VARS["PHP_SELF"]);
$server=$HTTP_SERVER_VARS["SERVER_NAME"].$directory;
$wlink="http://".$server."register/newreg.php?a=".$ums_user_id;

////////////////////////////////////////////////
//grundertrag
////////////////////////////////////////////////
//grundertragbonus f�r die BR, gibt es nie in der Ewigen Runde
if(($maxtick>2500000 && $sv_ewige_runde!=1 && $sv_hardcore!=1) OR ($sv_comserver==1 AND $sv_comserver_roundtyp==1)){
...
if(intval($_REQUEST['rh_amount'])>0 AND intval($_REQUEST['rh_cost']>0) AND hasTech($pt,4) AND $ally_has_notfallkonverter)
{
	...
	if (setLock($ums_user_id))
	{
		...
		if($res_target<1 OR $res_target>4)$res_target=1;
		
		//quellrohstoff auslesen
		$res_source=intval($_REQUEST['rh_v2']);
		if($res_source<1 OR $res_source>4)$res_source=1;
...
$e_t3=$_POST["e_t3"];
$e_t4=$_POST["e_t4"];
if(isset($e_t1) OR isset($e_t2) OR isset($e_t3) OR isset($e_t4)){
...
///////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////
if ($_POST["mtr"] OR $_POST["dtr"] OR $_POST["itr"] OR $_POST["etr"] OR $_POST["ttr"]){
	...
	if (setLock($ums_user_id)){
		...
		if(validDigit($mtr) AND validDigit($dtr) AND validDigit($itr) AND validDigit($etr) AND validDigit($ttr)){//alle werte sind ok
			...
			if ($ttr>$restyp05) $ttr=(int)$restyp05;
			  ...
			  for ($i=1; $i<=$b_col; $i++){
				  ...
				  if($sector==1 AND $colanz>25){
					...
					if ($z>0)
					{
					  ...
					  else mysql_query("UPDATE de_user_build SET anzahl = anzahl + '$z' WHERE user_id = '$ums_user_id' AND tech_id=80 AND verbzeit=4 ",$db);
					  ...
					  if($ownally!='')
					  {
						...
						$row = mysql_fetch_array($db_daten);    
						$allyid=$row['id'];
						if($row['questtyp']==0)mysql_query("UPDATE de_allys SET questreach=questreach+'$z' WHERE id='$allyid' AND questtyp=0", $db);
					  }
					}
...
if ($fehlermsg!='')echo '<div class="info_box"><span class="text2">'.$fehlermsg.'</span></div><br>';

if ($trademsg!='')echo '<div class="info_box"><span class="text1">'.$trademsg.'</span></div><br>';
