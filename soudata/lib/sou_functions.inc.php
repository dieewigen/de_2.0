...
function get_player_advertised($werberid)
{
  ...
  $num=0;
  if($werberid>0){
	//es z�hlen alle spieler, die in den letzten x tagen aktiv waren
	$time=time()-3600*24*3;
...
function get_baonadaskala()
{
  	...
  	for($i=1;$i<=6;$i++)
  	{ 
		//aktive spieler und deren ausr�stung f�r den bonus berechnen
...
function get_sector_owner($sbx, $sby)
{
  ...
  else //es gibt sonnensysteme
  {
    ...
    {
      //alle ss durchgehen und schauen welcher fraktion es geh�rt
    ...
    //maxwert auf einzigartigkeit �berpr�fen, d.h. wenn gr��er null
    if($max>0)
    {
      $anzmax=0;
      for($i=0;$i<=5;$i++)
      ...
      //wenn es nur einen maxwert gibt, dann den zur�ckgeben
      if($anzmax==1)
      {
        return($owner);
      }
...
function make_modul_info($row)
{
  ...
  {
    if($moduloutput!='')$moduloutput.='<br>';
  	$moduloutput.='Dieses Modul kann ein Geb&auml;ude der Stufe '.$row["canbldgupgrade"].' zu Stufe '.($row["canbldgupgrade"]+1).' aufwerten, wobei nat&uuml;rlich die Voraussetzungen f&uuml;r die n&auml;chste Geb&auml;udestufe erf&uuml;llt sein m&uuml;ssen. Bereits eingezahlte Rohstoffe werden verwendet. Das Modul wird bei der Verwendung leider zerst&ouml;rt. Zur Verwendung mu� sich das Modul an Bord eines Raumschiffes befinden.';
  }
...
function get_bldg_level($owner_id, $bldg_id)
{
  ...
  //�berpr�fen ob es bereits einen datensatz von dem geb�ude gibt
...
function has_systemhold_amount($uid, $res_id)
{
  ...
  //�berpr�fen ob es bereits einen datensatz mit der rohstoff-id gibt
...
//gesamte laderaumkapazit�t auslesen
function get_sum_hold($uid)
{
  global $soudb;
...
//�berpr�fen wie gro� die f�rderkapazit�t des schiffes ist
function get_canmine($uid)
{
  global $soudb;
