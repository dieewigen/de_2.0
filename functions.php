...
function sf($name, $arrOpt, $selected, $class="", $jsHandler =''){
	...
	foreach($arrOpt as $val => $show){
		...
		if($val == $selected) $field .= "selected=\"selected\"";
		$field .= ">$show</option>\n";
	}
	$field .= "</select>\n";
...
function showeinheit_ang($techname, $tech_id ,$rt01, $rt02, $rt03, $rt04, $rt05, $tech_ticks, $vorhanden, $bg, $tooltipid, $has_tech, $item_kosten=''){
	...
	if($tech_id==111)$va='id="va"';else $va='';
...
function showtech($techname, $gebnr,$rt01, $rt02, $rt03, $rt04, $rt05, $buildgtime, $buildgnr, $techs, $typ, $verbtime, $bg, $cancancel){
	...
	elseif ($techs[$gebnr]==1){ //schon gebaut/geforscht
		 if ($typ==0)echo '<td class="'.$bg.'">'.$functions['gebaut'].'</td>';
		 elseif ($typ==1)echo '<td class="'.$bg.'">'.$functions['erforscht'].'</td>';
		 else echo '<td class="'.$bg.'">'.$functions['gebaut'].'</td>';
	}
	elseif ($buildgnr>0){//es l�uft bereits etwas, keine freien Kapazitäten
		...
		if ($typ==0)echo '<td class="'.$bg.'"><a href="buildings.php?ida='.$gebnr.'&st='.$_SESSION['build_sec_token'].'" class="btn2">'.$functions['bauen'].'</a></td>';
		 	elseif($typ==1) echo '<td class="'.$bg.'"><a href="research.php?ida='.$gebnr.'&st='.$_SESSION['research_sec_token'].'" class="btn2">'.$functions['forschen'].'</a></td>';
		 	else echo '<td class="'.$bg.'"><a href="bkmenu.php?ida='.$gebnr.'" class="btn2">'.$functions['bauen'].'</a></td>';
	}
	echo "</tr>";
}
...
function showtech2($techname, $gebnr,$rt01, $rt02, $rt03, $rt04, $rt05, $buildgtime, $buildgnr, $techs, $typ, $verbtime, $bg, $cancancel){
	...
	else
	{
		 ...
		 //else echo '<td class="'.$bg.'"><input type="Submit" name="i'.$gebnr.'" value="'.$functions[forschen].'"></td>';
		if ($typ==0)echo '<td class="'.$bg.'"><a href="buildings.php?ida='.$gebnr.'&st='.$_SESSION['build_sec_token'].'" class="btn2">'.$functions['bauen'].'</a></td>';
		 	else echo '<td class="'.$bg.'"><a href="research.php?ida='.$gebnr.'&st='.$_SESSION['research_sec_token'].'" class="btn2">'.$functions['forschen'].'</a></td>';
		}
		echo "</tr>";
}
...
function get_fleet_ground_speed($ez, $rasse, $uid){
	...
	if ($schiffe_btkrest[0][0]>0) $rz1=$schiffsdaten[$rasse-1][0][0];//j�ger
	if ($schiffe_btkrest[0][1]>0) $rz1=$schiffsdaten[$rasse-1][5][0];//bomber
