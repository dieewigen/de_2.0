...
if (isset($_SESSION['ums_user_id']) && $_SESSION['ums_user_id'] > 0) {
    ...
    if (isset($GLOBALS['sv_log_player_actions']) && $GLOBALS['sv_log_player_actions'] == 1) {
        ...
        if (!isset($_REQUEST["check4new"]) and !isset($_REQUEST["managechat"])) {
            ...
            mysqli_execute_query($db_log, "INSERT INTO gameserverlogdata (serverid, userid, time, ip, file, getpost) VALUES(?, ?, NOW(), ?, ?, ?)", [$sv_servid, $_SESSION['ums_user_id'], $ip, $scriptname, $datenstring]);
    ...
    if (time() > $ic_activetime + $sv_activetime and $eftachatbotdefensedisable != 1 and $givenocredit != 1) {
        mysqli_execute_query($GLOBALS['dbi'], "UPDATE de_login SET activetime=? WHERE user_id=? AND status=1", [time(), $_SESSION['ums_user_id']]);
        mysqli_execute_query($GLOBALS['dbi'], "UPDATE de_user_data SET actpoints=actpoints+1 WHERE user_id=?", [$_SESSION['ums_user_id']]);
    }
    ...
    if ($eftachatbotdefensedisable != 1) { //kein chataufruf
        ...
        if ($_SESSION["aktivitaet_chat_time"] + 300 < time()) {
            $time = intval(date("H"));
            $zeit = date("Y-m-d");
            mysqli_execute_query($GLOBALS['dbi'], "UPDATE de_user_stat SET h{$time}='1' WHERE user_id=? AND datum=? AND h{$time}<'1'", [$_SESSION['ums_user_id'], $zeit]);
            $_SESSION["aktivitaet_chat_time"] = time();
        }
    }

    if ($_SESSION['ums_zeitstempel'] + 300 < time() && $eftachatbotdefensedisable != 1) { //nur alle 5 minuten aktualisieren
        mysqli_execute_query($GLOBALS['dbi'], "UPDATE de_login SET last_login=NOW() WHERE user_id=? AND status=1", [$_SESSION['ums_user_id']]);
