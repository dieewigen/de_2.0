...
$allytag=$row["allytag"];$newnews=$row["newnews"];$agent=$row["agent"];$sonde=$row["sonde"];$col=$row["col"];$col_build=$row["col_build"];
$status=$row["status"];$sector=$row["sector"];$system=$row["system"];$platz=$row["platz"];$rang_nr=$row["rang"];
$credits=$row["credits"];$actpoints=$row["actpoints"];$rundenpunkte=$row["roundpoints"];$kartefakt=$row["kartefakt"];
$kgget=$row["kgget"];$sou_user_id=$row["sou_user_id"];$geteacredits=$row["geteacredits"];$geteftabonus=$row["geteftabonus"];
$npcartefact=$row['npcartefact'];$ally_tronic=$row['ally_tronic'];$eh_counter=$row['eh_counter'];
...
for ($j=0;$j<=6;$j++){
	if(!isset($ovoptfelder[$j])){
	  ...
	  if($sv_ewige_runde==1){
			...
			$sc1=7;
			$sc2=8;
			if($p>0.33){$sc1=4;$sc2=5;}
			if($p>0.66){$sc1=1;$sc2=2;}
    ...
    case 6://schiffseinheiten
      ...
      xecho ($ueberschrift.'');
       ...
       $ec81=0;$ec82=0;$ec83=0;$ec84=0;$ec85=0;$ec86=0;$ec87=0;$ec88=0;$ec89=0;$ec90=0;
    ...
    case 8://schiffseinheiten/verteidigungsanlagen
		...
		//zaehle alle schiffe, die schon vorhanden sind - anfang
		$ec=array();
		 $ec[81]=0;$ec[82]=0;$ec[83]=0;$ec[84]=0;$ec[85]=0;$ec[86]=0;$ec[87]=0;$ec[88]=0;$ec[89]=0;$ec[90]=0;
		 $fid0=$ums_user_id.'-0';$fid1=$ums_user_id.'-1';$fid2=$ums_user_id.'-2';$fid3=$ums_user_id.'-3';
		 $db_daten=mysql_query("SELECT e81, e82, e83, e84, e85, e86, e87,e88,e89,e90 FROM de_user_fleet WHERE user_id='$fid0' OR user_id='$fid1' OR user_id='$fid2' OR user_id='$fid3'ORDER BY user_id ASC",$db);
		while($row = mysql_fetch_array($db_daten)){
			$ec[81]+=$row['e81'];
		...
		//zaehle alle verteidigungsanlagen, die schon vorhanden sind - anfang
		$ec[100]=0;$ec[101]=0;$ec[102]=0;$ec[103]=0;$ec[104]=0;
    ...
    //}
        ...
        if($do_calc==1 AND $ac_max>0)
        {
          ...
          $sc2=11;
      	  ...
      	  if($p>0.33){$sc1=7;$sc2=8;}
      	  if($p>0.66){$sc1=4;$sc2=5;}
      	  if($p>=1){$sc1=1;$sc2=2;$sc3=2;$sc4=3;}
        ...
        $str1='</font>';
        if($ac_akt<$ac_max){$str='<font color="#FF0000">';}else{$str='<font color="#00FF00">';}
