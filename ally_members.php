...
while ($row < $nb){
    ...
    echo '<form name="f'.$sector."x".$system.'" action="sector.php?sf='.$sectorjump.'" method="POST">';
    echo         "<tr>\n".
                    "<td class=\"cl\"><a href=\"details.php?SID=$SID&a=s&se=$sector&sy=$system\">".utf8_encode($name)."</a></td>\n".
	...
	echo '<td class="cc">'.($relrang+1).'</td>';
     ...
     //echo "<td class=\"cr\">".$activity."</td>\n";
        ...
        if ($isleader)
                echo "<td class=\"cr\"><a onClick=\"return confirm('$allymembers_lang[msg_1_1] $name $allymembers_lang[msg_1_2]');\" href=\"ally_kick.php?userid=$userid\"><font face=\"tahoma\" style=\"font-size:8pt;\">$allymembers_lang[entlassen]</font></a></td>\n".
