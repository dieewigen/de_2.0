...
class GetAlliances
{
    ...
    const string GET_ALLIANCES = "SELECT id, allytag, allyname, leaderid, memberlimit, artefacts, t_depot FROM de_allys";
    ...
    public function getAlliances()
    {
        ...
        foreach ($rows as $row) {
            ...
            $artefacts = $leaderCount > 0 ? $row['artefacts'] : 0; // don't return artefacts of player alliances
            $tronics = $leaderCount > 0 ? $row['t_depot'] : 0; // don't return tronic of player alliances
            $result[] = new AllianceEntry($row['id'], $row['allytag'], $row['allyname'], $leaderCount > 0, $openSlots,
            $row['leaderid'], $artefacts, $tronics);
        }
        return $result;
    }
}
