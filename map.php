	...
	if($sec_data['npc']==1){
		...
		while($row = mysqli_fetch_array($db_daten)){
			...
			*/
			...
			if(strtotime($row["last_login"])+1800 > time() AND $row["lstatus"]==1) $os='&nbsp;*';else $os='';
		...
		if($anz>0){
			...
			if($sec_data['name']!='')$sektorinfo.=' - '.utf8_decode($sec_data['name']);
			$sektorinfo.='</span>';
		}else{
			$sektorinfo='';
			$sektorinfo='';
	...
	if($ums_premium==1){
		...
		for($we=0; $we<5; $we++){
			...
			if($npcsec==0){
	  			...
	  			if($col_angriffsgrenze>$sv_max_col_attgrenze)$col_angriffsgrenze=$sv_max_col_attgrenze;
	  			if($col_angriffsgrenze<$sv_min_col_attgrenze)$col_angriffsgrenze=$sv_min_col_attgrenze;
	  		}else{
