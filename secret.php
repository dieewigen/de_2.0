...
$punkte=$row["score"];$newtrans=$row["newtrans"];$newnews=$row["newnews"];$sonde=$row["sonde"];
$agent=$row["agent"];$sector=$row["sector"];$system=$row["system"];
$gr01=$restyp01;$gr02=$restyp02;$gr03=$restyp03;$gr04=$restyp04;
$scanhistory=$row['scanhistory'];$mysc1=$row["sc1"];$mysc2=$row["sc2"];$mysc3=$row["sc3"];$mysc4=$row["sc4"];
...
if($artbonusbuild>6)$artbonusbuild=6;
...
if(hasTech($pt,9)){
	if (isset($_POST["b110"]) || isset($_POST["b111"])){//ja, es wurde ein button gedrueckt
		...
		else echo '<br><font color="#FF0000">'.$secret_lang['transerror2'].'</font><br><br>';
...
if(!hasTech($pt,9)){
	...
	if(!empty($zsec1)){
		if($zsec1==1 OR $ownsector==1)$zsec1='-1';
	}
	
	if(!empty($zsec2)){
		if($zsec2==1 OR $ownsector==1)$zsec2='-1';
	}
	...
	if((isset($_GET["a"]) && $_GET["a"]=='d') || $showscanhistory==1){
		...
		if($num==1)//die koordinaten stimmen, gib die daten aus
		{
		  ...
		  //rasse
		  $rasse='?';
		  if ($row["rasse"]==1)$rasse='Ewiger';
		  elseif ($row["rasse"]==2)$rasse='Ishtar';
		  elseif ($row["rasse"]==3)$rasse='K&#180;Tharr';
		  elseif ($row["rasse"]==4)$rasse='Z&#180;tah-ara';
		  elseif ($row["rasse"]==5)$rasse='DX61a23';
		  //allianz
		  $allianz='?';if ($row["allytag"]!='')$allianz=$row["allytag"];
		  if($row["atime"]>0) $allianz.=' ('.date("d.m.Y - G:i", $row["atime"]).')';
		  //schiffs�bersicht
		  $ftime='?';if($row["ftime"]>0) $ftime=date("d.m.Y - G:i", $row["ftime"]);
		  $e81='?';if($row["ftime"]>0) $e81=number_format($row["e81"], 0,"",".");
		  $e82='?';if($row["ftime"]>0) $e82=number_format($row["e82"], 0,"",".");
		  $e83='?';if($row["ftime"]>0) $e83=number_format($row["e83"], 0,"",".");
		  $e84='?';if($row["ftime"]>0) $e84=number_format($row["e84"], 0,"",".");
		  $e85='?';if($row["ftime"]>0) $e85=number_format($row["e85"], 0,"",".");
		  $e86='?';if($row["ftime"]>0) $e86=number_format($row["e86"], 0,"",".");
		  $e87='?';if($row["ftime"]>0) $e87=number_format($row["e87"], 0,"",".");
		  $e88='?';if($row["ftime"]>0) $e88=number_format($row["e88"], 0,"",".");
		  $e89='?';if($row["ftime"]>0) $e89=number_format($row["e89"], 0,"",".");
		  $e90='?';if($row["ftime"]>0) $e90=number_format($row["e90"], 0,"",".");

		  //turmübersicht
		  $dtime='?';if($row["dtime"]>0) $dtime=date("d.m.Y - G:i", $row["dtime"]);
		  $e100='?';if($row["dtime"]>0) $e100=number_format($row["e100"], 0,"",".");
		  $e101='?';if($row["dtime"]>0) $e101=number_format($row["e101"], 0,"",".");
		  $e102='?';if($row["dtime"]>0) $e102=number_format($row["e102"], 0,"",".");
		  $e103='?';if($row["dtime"]>0) $e103=number_format($row["e103"], 0,"",".");
		  $e104='?';if($row["dtime"]>0) $e104=number_format($row["e104"], 0,"",".");

		  //sondenbericht
		  $stime='?';if($row["stime"]>0) $stime=date("d.m.Y - G:i", $row["stime"]);
		  $score='?';if($row["stime"]>0) $score=number_format($row["score"], 0,"",".");
		  $fleet='?';if($row["stime"]>0) $fleet=number_format($row["fleet"], 0,"",".");
		  $defense='?';if($row["stime"]>0) $defense=number_format($row["defense"], 0,"",".");
		  $build='?';if($row["stime"]>0) $build=number_format($row["build"], 0,"",".");
		  $col='?';if($row["stime"]>0) $col=number_format($row["col"], 0,"",".");
		  $buildings='?';if($row["stime"]>0) $buildings=number_format($row["buildings"], 0,"",".");
		  $restyp01='?';if($row["stime"]>0) $restyp01=number_format($row["restyp01"], 0,"",".");
		  $restyp02='?';if($row["stime"]>0) $restyp02=number_format($row["restyp02"], 0,"",".");
		  $restyp03='?';if($row["stime"]>0) $restyp03=number_format($row["restyp03"], 0,"",".");
		  $restyp04='?';if($row["stime"]>0) $restyp04=number_format($row["restyp04"], 0,"",".");
		  $restyp05='?';if($row["stime"]>0) $restyp05=number_format($row["restyp05"], 0,"",".");
	...
	if(validDigit($zsec1)&&validDigit($zsys1)&&$sonde>0){
	  ...
	  //�berpr�fen ob evtl. der schild des herakles vorhanden ist
	  $herakles=0;
	  ...
	  if($num==1 AND $herakles==0){//die koordinaten stimmen, gib die daten aus
		...
		if($row['npc']==0){
			$uid=$row["user_id"]; //hole die user_id des users um die daten anfordern zu k�nnen
			$zpunkte=$row["score"];$vertanz=$row["e100"]+$row["e101"]+$row["e102"]+$row["e103"]+$row["e104"];
			...
			$row = mysqli_fetch_array($db_daten);
			if(strtotime($row["last_login"])+43200 > time()) $isonline=$secret_lang['ja'];else $isonline=$secret_lang['nein'];
			if($npc==1)$isonline=$secret_lang['unbekannt'];
			...
			if (hasTech($zpt,12)) $w=45;
			else if (hasTech($zpt,11)) $w=30;
			else if (hasTech($zpt,10)) $w=15;
			else $w=0;
			//mt_srand(10000);
			...
			else echo '<div class="info_box text2">Die Technologie der DX61a23 ist zu weit fortgeschritten um dort Informationen sammeln zu k&ouml;nnen.</div><br>';		
		}
		else echo '<div class="info_box text2">'.$secret_lang['falschezielkoords'].'</div><br>';

	} else echo '<<div class="info_box text2">'.$secret_lang['falschewerte'].'</div><br>';
	...
	if(validDigit($zsec2)&&validDigit($zsys2)&&validDigit($az)){
		...
		if ($num==1&&$az>0&&$zk<>$ak){ //die koordinaten stimmen
			...
			if ($sector==$zsec2){//gleicher sektor und man benoetigt nur plan. boerse
				if (hasTech($pt,3) AND hasTech($zpt,3)) $ok=1; else $ok=3;
			}
			
			if ($sector<>$zsec2){//anderer sektor, beide benötigen gilde
				if (hasTech($pt,4) and hasTech($zpt,4)) $ok=1; else $ok=3;
			}
	  ...
	  if ($ok==2)echo '<div class="info_box text2">'.$secret_lang['einsatzfehlerhaft'].'</div><br>';
	  if ($ok==3){
	  ...
	  if ($ok==1){//die koordinaten stimmen und es werden agenten geschickt
		...
		if ($ums_rasse==1 AND $rasse==1)$bomalus=0;
		if ($ums_rasse==1 AND $rasse==2)$bomalus=5;
		if ($ums_rasse==1 AND $rasse==3)$bomalus=0;
		if ($ums_rasse==1 AND $rasse==4)$bomalus=-5;
		if ($ums_rasse==1 AND $rasse==5)$bomalus=-10;

		if ($ums_rasse==2 AND $rasse==1)$bomalus=-5;
		if ($ums_rasse==2 AND $rasse==2)$bomalus=0;
		if ($ums_rasse==2 AND $rasse==3)$bomalus=-5;
		if ($ums_rasse==2 AND $rasse==4)$bomalus=-10;
		if ($ums_rasse==2 AND $rasse==5)$bomalus=-10;

		if ($ums_rasse==3 AND $rasse==1)$bomalus=0;
		if ($ums_rasse==3 AND $rasse==2)$bomalus=5;
		if ($ums_rasse==3 AND $rasse==3)$bomalus=0;
		if ($ums_rasse==3 AND $rasse==4)$bomalus=-5;
		if ($ums_rasse==3 AND $rasse==5)$bomalus=-10;

		if ($ums_rasse==4 AND $rasse==1)$bomalus=5;
		if ($ums_rasse==4 AND $rasse==2)$bomalus=10;
		if ($ums_rasse==4 AND $rasse==3)$bomalus=5;
		if ($ums_rasse==4 AND $rasse==4)$bomalus=0;
		if ($ums_rasse==4 AND $rasse==5)$bomalus=-10;

		if ($ums_rasse==5 AND $rasse==1)$bomalus=10;
		if ($ums_rasse==5 AND $rasse==2)$bomalus=10;
		if ($ums_rasse==5 AND $rasse==3)$bomalus=10;
		if ($ums_rasse==5 AND $rasse==4)$bomalus=10;
		if ($ums_rasse==5 AND $rasse==5)$bomalus=10;

		//test ob der einsatz erfolgreich ist
		if ($zagent==0) $w=98;
		else $w = ($az+($az*$artbonusatt/100))/($zagent+($zagent*$zartbonusdeff/100))*(100+$bomalus);
		if ($w>98) $w=98;
		...
		if($w>$sv_artefakt[20][5]){
			...
			if(mysqli_num_rows($db_daten)==1)$w=$sv_artefakt[20][5];
		}
		...
		if ($w>=$r){ //der einsatz klappt
		  ...
		  switch($etyp){
			...
			case 1: //flottenauftrag
			  ...
			  if ($a1==0) $a1=$secret_lang['systemverteidigung'];
			  elseif ($a1==1) $a1=$secret_lang['angriffreisezeit'].$t1;
			  elseif ($a1==2) $a1=$secret_lang['verteidigung'].$zsec1x.':'.$zsys1.$secret_lang['reisezeit'].$t1;
			  elseif ($a1==3) $a1='&nbsp;&nbsp;'.$secret_lang['rueckflug'].'&nbsp;&nbsp;'.$secret_lang['reisezeit2'].$t1;
			  //elseif ($a1==4) $a1='&nbsp;&nbsp;'.$secret_lang[archaeologie].'&nbsp;&nbsp;'.$secret_lang[reisezeit2].$t1;
			  elseif ($a1==4) $a1='&nbsp;&nbsp;Mission bis: '.date("H:i:s d.m.Y",$mission_time1);

			  if ($a1[0]=='V' && $t1==0) $a1=$secret_lang['verteidige'].$zsec1x.':'.$zsys1.$secret_lang['zeit'].$at1;

			  if ($a2==0) $a2=$secret_lang['systemverteidigung'];
			  elseif ($a2==1) $a2=$secret_lang['angriffreisezeit'].$t2;
			  elseif ($a2==2) $a2=$secret_lang['verteidigung'].$zsec2x.':'.$zsys2.$secret_lang['reisezeit'].$t2;
			  elseif ($a2==3) $a2='&nbsp;&nbsp;'.$secret_lang['rueckflug'].'&nbsp;&nbsp;'.$secret_lang['reisezeit2'].$t2;
			  //elseif ($a2==4) $a2='&nbsp;&nbsp;'.$secret_lang[archaeologie].'&nbsp;&nbsp;'.$secret_lang[reisezeit2].$t2;
			  elseif ($a2==4) $a2='&nbsp;&nbsp;Mission bis: '.date("H:i:s d.m.Y",$mission_time2);

			  if ($a2[0]=='V' && $t2==0) $a2=$secret_lang['verteidige'].$zsec2x.':'.$zsys2.$secret_lang['zeit'].$at2;

			  if ($a3==0) $a3=$secret_lang['systemverteidigung'];
			  elseif ($a3==1) $a3=$secret_lang['angriffreisezeit'].$t3;
			  elseif ($a3==2) $a3=$secret_lang['verteidigung'].$zsec3x.':'.$zsys3.$secret_lang['reisezeit'].$t3;
			  elseif ($a3==3) $a3='&nbsp;&nbsp;'.$secret_lang['rueckflug'].'&nbsp;&nbsp;'.$secret_lang['reisezeit2'].$t3;
			  //elseif ($a3==4) $a3='&nbsp;&nbsp;'.$secret_lang[archaeologie].'&nbsp;&nbsp;'.$secret_lang[reisezeit2].$t3;
			  elseif ($a3==4) $a3='&nbsp;&nbsp;Mission bis: '.date("H:i:s d.m.Y",$mission_time3);

			  if ($a3[0]=='V' && $t3==0) $a3=$secret_lang['verteidige'].$zsec3x.':'.$zsys3.$secret_lang['zeit'].$at3;
			...
			case 3: //nachrichten
			  ...
			  while($row = mysql_fetch_array($db_daten)) //jeder gefundene datensatz wird ausgegeben
			  {
				...
				switch($n){
				  ...
				  default:
					$dontshow=0;
					if ($n==51 AND strpos ($row["text"], "($sector:$system)")=== false ) $dontshow=1;

					if ($dontshow==0){
					  //atter/deffer farbig darstellen
					  $hstr1='';$hstr2='';
					  if($n==51){$hstr1='<font color="#FF0000">';$hstr2='</font>';}
					  if($n==52){$hstr1='<font color="#FF0000">';$hstr2='</font>';}
					  if($n==53){$hstr1='<font color="#00FF00">';$hstr2='</font>';}
					  if($n==54){$hstr1='<font color="#00FF00">';$hstr2='</font>';}
					  echo '<tr>';
					  echo '<td class="cl">'.$time.'</td>';
					  echo '</tr>';
					  echo '<tr>';
			...
			case 5: //allytag
				...
				//die daten in de_user_scan hinterlegen
				if ($allytag=='' OR $row["status"]!=1) $saveallytag='';else $saveallytag=$allytag;
				for($i=0;$i<count($savelist);$i++){
					$save_uid=$savelist[$i];
				...
				if ($allytag=='' OR $row["status"]!=1) $allytag=$secret_lang['keineally'];
		  ...
		  $showmenu=1;
			...
			if ($aze==1)$msg=$secret_lang['einsatzentdeckt'].$ums_spielername.' ('.$sector.':'.$system.$secret_lang['einsatzentdeckt2'].$aze.$secret_lang['einsatzentdeckt3'];
			else $msg=$secret_lang['einsatzentdeckt'].$ums_spielername.' ('.$sector.':'.$system.$secret_lang['einsatzentdeckt2'].$aze.$secret_lang['einsatzentdeckt4'];
			  ...
			  mysql_query("update de_user_data set scanhistory='$entry' where user_id = $ums_user_id",$db);
				...
				if($drin=="0"){
					while($i<Count($einsaetze)-1){
						if($i<4)$entry=$entry.''.$einsaetze[$i][0].':'.$einsaetze[$i][1].':'.$einsaetze[$i][2].'|';
						$i++;
					}
					$entry=utf8_decode($entry);
					mysql_query("update de_user_data set scanhistory='$entry' where user_id = $ums_user_id",$db);
	...
	if ($sonde>0 AND hasTech($pt,9) AND hasTech($pt,110)){//stelle sondenmen� dar
	  ...
	  if(hasTech($pt,111)){//stelle agentenmen� dar
		...
		if($_REQUEST['etyp']==7){$selected=' selected';}else{$selected='';}
		echo '<option value="7"'.$selected.'>'.$secret_lang['sabotage_kollektoroutput'].'</option>';
		if($_REQUEST['etyp']==8){$selected=' selected';}else{$selected='';}
		...
		if($_REQUEST['etyp']==10){$selected=' selected';}else{$selected='';}
	...
	<?php
	  ...
	  while($row = mysql_fetch_array($db_daten)) {
		...
		if(hasTech($pt, $tech_id)){ //echo "Vorbedingung erf�llt";
			...
			showeinheit2(getTechNameByRasse($row["tech_name"],$_SESSION['ums_rasse']), $tech_id, 
				...
				$einheiten_daten[$tech_id]['bz'], $ec, $bg, $z);
	...
	function sabotageallowed($zuid){
	  ...
	  $znpc=$row["npc"];
	  $zspec5=$row['spec5'];
	  if($row["allytag"]!='' AND $row["status"]==1)$zallytag=$row["allytag"];else $zallytag='';
	  ...
	  if ($at!=0)
	  {
		...
		{
		  if ($ownally != $row["allytag"]) $allypartner[] = $row["allytag"];
		}
	  }

	  if (($ownally!='') AND (($ownally==$zallytag) OR (in_array($zallytag, $allypartner)))) $sabotageallowed=0;

	  if($znpc==1)$sabotageallowed=0;
	  //spezialisierung, kann nicht sabotiert werden
	  if($zspec5==1)$sabotageallowed=0;
