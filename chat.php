...
if(!isset($_SESSION["de_chat_inputchannel"]))$_SESSION["de_chat_inputchannel"]=0;
...
//die Textgr��e in der mobilen Version anpassen
if($_SESSION['ums_mobi']==1){
	/*
	$('#chatcontent').css('font-size', '28px');
	document.styleSheets[0].addRule('a', 'font-size: 28px;');
	document.styleSheets[0].addRule('a:visited', 'font-size: 28px;');
	document.styleSheets[0].addRule('a:link', 'font-size: 28px;');
	document.styleSheets[0].addRule('a:hover', 'font-size: 28px;');*/
...
<?php 
	if($ums_user_id==1){
		//echo 'alert("de_ajaxrpc.php?chatinsert=1&insert="+inputfield);';
	}
?>  
  
  $.post("de_ajaxrpc.php?chatinsert=1&insert="+inputfield, 
