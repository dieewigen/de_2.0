...
include 'inc/lang/'.$sv_server_lang.'_secforumvt.lang.php';
...
<!doctype html>
  <html>
    <head>
      <title><?=$secforumvt_lang[title]?></title>
    ...
    <body>
      ...
      <?//stelle die ressourcenleiste dar
        ...
        $temporary[gelesen] = substr_replace($temporary[gelesen], "1", $system, 1);
        mysql_query("UPDATE de_sectorforum_threads set gelesen='$temporary[gelesen]' WHERE id='$id'");

        if("$sector"<>"$temporary[sector]")
        {
          echo $secforumvt_lang[msg_1];
          exit();
        }

        echo '<a name="oben">&nbsp;</a><br>
              <a href="javascript:history.back()"><img src="'.$ums_gpfad.'g/'.$ums_rasse.'_goback.gif" border="0" alt="'.$secforumvt_lang[zurueck].'"></a>
              <a href="#unten"><img src="'.$ums_gpfad.'g/'.$ums_rasse.'_godown.gif" border="0" alt="'.$secforumvt_lang[tolastpost].'"></a><br><br>
              <font size="2" face="arial"><b><a href="secforum.php">'.$secforumvt_lang[title].'</a></b> -> <b>'.$secforumvt_lang[thema].': '.$temporary[threadname].'</b></font><br><br>';
        ...
        if($reply)
        {
          ...
          if(!$_POST[nachricht])$error.=$secforumvt_lang[msg_3];
          if(!$_POST[title])$error.=$secforumvt_lang[msg_4];
          if(!$ums_user_id)$error.=$secforumvt_lang[msg_5];
          ...
          if($num==0)$error.=$secforumvt_lang[msg_6];
          $row = mysql_fetch_array($db_daten);
          if ($sector!=$row[sector])$error.=$secforumvt_lang[msg_7];
	...
	elseif(!$error)
      ...
      {
        $newstext=$row[post];
        $posttime =date ("d.m.Y $secforumvt_lang[msg_8_4] H:i", $row[time]);
        $newstext = formatierte_anzeige($newstext,$ums_gpfad);

        if($row[title]=="")
        {
          $row[title]=$secforumvt_lang[re].":";
          ...
          <td width="100%" bgcolor="#202020" align="left">
              ...
              <tr>
                  <td>
                      ...
                      <font face="Arial" size="2">- '.$secforumvt_lang[msg_8_1].' '.$row[poster].' '.$secforumvt_lang[msg_8_2].' '.$posttime.' '.$secforumvt_lang[msg_8_3].'</font>
                  </td>
                  <td>';
                  if($row[poster]==$ums_spielername)
                  echo '<a href="secforumvt.php?action=edit&id='.$row[thread].'&pid='.$row[postid].'&#unten">'.$secforumvt_lang[edit].'</a>';
                  else
                  echo '&nbsp;';
...
echo '            </td>
        ...
        echo '<tr><td bgcolor="#202020" align="left"><font size="-2">&nbsp;'.$secforumvt_lang[msg_9_1].' '.$male.' '.$secforumvt_lang[msg_9_2].' '.$time.'.</font></td></tr>';
        }
echo '</table><br>';
}
echo '<a name="unten">&nbsp;</a><font size="2" face="arial"><b><a href="secforum.php">'.$secforumvt_lang[title].'</a></b> -> <b>'.$secforumvt_lang[thema].': '.$temporary[threadname].'</b></font><br><br>

<a href="javascript:history.back()"><img src="'.$ums_gpfad.'g/'.$ums_rasse.'_goback.gif" border="0" alt="'.$secforumvt_lang[zurueck].'"></a>
<a href="#oben"><img src="'.$ums_gpfad.'g/'.$ums_rasse.'_goup.gif" border="0" alt="'.$secforumvt_lang[tofirstpost].'"></a><br><br>';
...
<td class="ro" align="center"><?=$secforumvt_lang[vorschau]?></td>
...
{
$db_edit=mysql_unbuffered_query("SELECT poster FROM de_sectorforum_posts WHERE postid='$pid'");
$rew = mysql_fetch_array($db_edit);
}
...
?>
<form method="POST" action="secforumvt.php?id=<?=$id?>&#unten">
...
<?
if($action=="edit")
echo $secforumvt_lang[beitrageditieren].'</div></td>';
else
echo $secforumvt_lang[antwortauf].' '.$temporary[threadname].' '.$secforumvt_lang[erstellen].'</div></td>';
?>
    <td width="13" height="25" class="ror"></td>
</tr>
<tr>
    <td width="13" height="25" class="rl"></td>
    <td class="tl"><?=$secforumvt_lang[titel]?>:</td>
...
echo 'value="'.$secforumvt_lang[re].':'.$temporary[threadname].'"';
?>
...
<tr>
<td width="13" height="25" class="rl"></td>
<td class="tl" valign="top"><?=$secforumvt_lang[nachricht]?>:</td>
...
<?
if($action=="edit")
echo '<input type="submit" value="'.$secforumvt_lang[aenderungspeichern].'"  name="savedit"><input type="hidden" name="pid" value="'.$pid.'">&nbsp;<input type="submit" value="'.$secforumvt_lang[vorschau].'"  name="vorschauedit"></div></td>';
else
echo '<input type="submit" value="'.$secforumvt_lang[antworten].'"  name="reply">&nbsp;<input type="submit" value="'.$secforumvt_lang[vorschau].'"  name="vorschau"></div></td>';
?>
...
{
echo '<br><br><a href="secforum.php?a=c&threadid='.$id.'">'.$secforumvt_lang[threadschliessen].'</a> - <a href="secforum.php?a=d&threadid='.$id.'" onclick="return confirm(\''.$secforumvt_lang[msg_10].'\')">'.$secforumvt_lang[threadloeschen].'</a> - ';

if($temporary[wichtig]=="0")
echo '<a href="secforum.php?a=ota&threadid='.$id.'">'.$secforumvt_lang[ontopaktivieren].'</a>';
else
echo '<a href="secforum.php?a=otd&threadid='.$id.'">'.$secforumvt_lang[ontopdeaktivieren].'</a>';
}
}
else
{
...
if($system==issectorcommander())
{
echo '<br><br><a href="secforum.php?a=o&threadid='.$id.'">'.$secforumvt_lang[threadoeffnen].'</a> - <a href="secforum.php?a=d&threadid='.$id.'" onclick="return confirm(\''.$secforumvt_lang[msg_10].'\')">'.$secforumvt_lang[threadloeschen].'</a> - ';
