...
<div style="width:100%; font-size:40pt; color:#FF0000;font-family: Impact;
    filter:Glow(color=#00FF00, strength=12)">EA Umfragen</div>
...
mysql_query("UPDATE sou_vote_umfragen SET status = 1, startdatum='$time' WHERE id='$id'", $soudb);
echo "<h2>Vote aktiviert</h2>";

$anzahlantworten = mysql_query("Select antworten from sou_vote_umfragen WHERE id='$id'", $soudb);
...
{
mysql_query("INSERT INTO sou_vote_stimmen (user_id, vote_id, votefor) VALUES (0,'$id','$i')", $soudb);
$i++;
}
...
$endstimmen = array();
$counter=0;
$db_endstimmen=mysql_query("Select count(votefor), votefor from sou_vote_stimmen  where vote_id='$id' group by votefor", $soudb);
...
mysql_query("UPDATE sou_vote_umfragen SET status=2, enddatum='$time', ergebnisse='$db_ergebnisse', stimmen='$stimmen' WHERE id='$id'", $soudb);
echo "<h2>Vote deaktiviert</h2>";


}

if($subentry && $frage!="" && $hinweis!="")
{
$a1 = str_replace("|","",$a1);
$a2 = str_replace("|","",$a2);
$a3 = str_replace("|","",$a3);
...
mysql_query("INSERT INTO sou_vote_umfragen(frage, antworten, hinweis, status) VALUES ('$frage', '$antworten','$hinweis',0)",$soudb);
...
mysql_query("UPDATE sou_vote_umfragen SET frage = '$frage', antworten='$antworten', hinweis='$hinweis' WHERE id='$id'", $soudb);
...
if($addant)
{
$db_umfrage=mysql_query("SELECT antworten FROM sou_vote_umfragen where id='$id'", $soudb);
$row = mysql_fetch_array($db_umfrage);

$antworten = "$row[antworten]|$wantwort";

mysql_query("UPDATE sou_vote_umfragen SET antworten='$antworten' WHERE id='$id'", $soudb);
...
if($delvote)
{
mysql_query("Delete From sou_vote_umfragen WHERE id='$id'", $soudb);
...
<?
$db_umfrage=mysql_query("SELECT id, frage, status FROM sou_vote_umfragen where status=0 order by id", $soudb);
while($row = mysql_fetch_array($db_umfrage))
{
echo '<tr align="center"><td class="rl" width="13">&nbsp;</td>';
echo '<td>'.$row[id].'</td><td><a href="sou_umfragen.php?action=edit&id='.$row[id].'">'.$row[frage].'</a></td><td>';
echo '<a href="sou_umfragen.php?action=ak&id='.$row[id].'" onclick="return confirm(\'M&ouml;chtes du diese Umfrage wirklich aktivieren?\')"><font color="#FF8000">inaktiv</font></a> </td><td class="rr" width="13">&nbsp;</td></tr>';
}
?>
...
<?
$db_umfrage=mysql_query("SELECT id, frage, status FROM sou_vote_umfragen where status=1 order by id", $soudb);
while($row = mysql_fetch_array($db_umfrage))
{
echo '<tr align="center"><td class="rl" width="13">&nbsp;</td>';
echo '<td>'.$row[id].'</td><td><a href="sou_umfragen.php?action=tendenz&id='.$row[id].'">'.$row[frage].'</a></td><td>';
echo '<a href="sou_umfragen.php?action=deak&id='.$row[id].'" onclick="return confirm(\'M&ouml;chtes du diese Umfrage wirklich beenden?\')"><font color="#00FF00">offen</font></a> </td><td class="rr" width="13">&nbsp;</td></tr>';
}
?>
...
<?
$db_umfrage=mysql_query("SELECT id, frage, status FROM sou_vote_umfragen where status=2 order by id", $soudb);
while($row = mysql_fetch_array($db_umfrage))
{
echo '<tr align="center"><td class="rl" width="13">&nbsp;</td>';
echo '<td>'.$row[id].'</td><td><a href="sou_umfragen.php?action=show&id='.$row[id].'">'.$row[frage].'</a></td><td>';
...
if($action=="edit")
{
$db_umfrage=mysql_query("SELECT id, frage, hinweis, antworten, status FROM sou_vote_umfragen where id='$id'", $soudb);
$row = mysql_fetch_array($db_umfrage);

?>
<form action="sou_umfragen.php" methode="post">
...
<tr>
 <td>Frage:</td>
 <td><input type="text" name="frage" size="48" value="<? echo "$row[frage]\""; if($row[status]!=0)echo 'disabled'; ?>></td>
</tr>
<tr>
 <td valign="top">Hinweis:</td>
 <td><textarea name="hinweis" cols="30" rows="8" <? if($row[status]!=0)echo 'disabled'; ?>><? echo $row[hinweis]; ?></textarea></td>
</tr>
<?
$i=0;
...
<form action="sou_umfragen.php?action=edit" methode="post">
<?
...
if($action=="show")
{
$db_checkobende=mysql_query("Select frage, antworten, hinweis, stimmen, status, startdatum, enddatum, ergebnisse from sou_vote_umfragen where status=2 and id='$id'", $soudb);
...
if($action=="tendenz")
{
$db_vorabdaten=mysql_query("Select frage, antworten, hinweis, stimmen, status, startdatum, ergebnisse from sou_vote_umfragen where status=1 and id='$id'", $soudb);
...
$counter=0;
$db_stimmen=mysql_query("Select count(votefor), votefor from sou_vote_stimmen  where vote_id='$id' group by votefor", $soudb);
