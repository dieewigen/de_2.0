...
$uid = req_int('uid');
$recall = req_str('recall');

// Zustandsändernde Aktion: Token prüfen, bevor irgendetwas ausgegeben wird
if ($recall !== '') {
    csrf_require();
}

$page_title = 'Sektorstatus';
...
if ($uid > 0)
{
  ...
  if ($recall !== '')//flotte nach daheim verlegen
  {
    ...
    echo '<div class="flash flash-ok">Alle Flotten wurden ins Heimatsystem verlegt.</div>';
  }

  //recall-button für alle flotten des spielers
  echo '<form action="sektorstatus.php" method="get" data-confirm="Wirklich alle Flotten von (' . $sector . ':' . $system . ') in das Heimatsystem verlegen?">';
  echo csrf_field();
...
<th width="70" class="ro">Ziel</th>
<th width="80" class="ro">System</th>
<th width="200" class="ro">Status</th>
<th width="50" class="ro">Zeit</th>
<th width="100" class="ro">Schiffe</th>
