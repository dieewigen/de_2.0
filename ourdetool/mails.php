...
$page_title = 'eMail an User senden';
include "inc.layout.top.php";
$active_usertab = 'mails';
...
function SetMsg(NewMsg) {
 ...
 switch(NewMsg) {
  case "Multi":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen Multi wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen Multiverstoß gesperrt.\r\nDa ein Multiaccount ein Verstoß gegen die Nutzungsbedingungen ist, bleiben die Accounts gesperrt, bis diese nach Ablauf der Inaktivenfrist (xDE = 7 Tage, SDE = 7 Tage) gelöscht werden.\r\nBis dahin darf KEIN neuer Account erstellt werden, da dieser sonst erneut als Multi gesperrt und gelöscht werden würde.\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "AFarming":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen Aufruf zum Farming wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen aufruf zum Farming gesperrt.\r\nDa dies ein Verstoß gegen die Nutzungsbedingungen ist, bleibt der Account gesperrt, bis dieser nach Ablauf der Inaktivenfrist (xDE = 7 Tage, SDE = 7 Tage) gelöscht wird.\r\nBis dahin darf KEIN neuer Account erstellt werden, da dies sonst als Multi behandelt wird!\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "Farming":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen Farming wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen Farmings mit dem Account '' gesperrt.\r\nDa dies ein Verstoß gegen die Nutzungsbedingungen ist, bleibt der Account gesperrt, bis dieser nach Ablauf der Inaktivenfrist (xDE = 7 Tage, SDE = 7 Tage) gelöscht wird.\r\nBis dahin darf KEIN neuer Account erstellt werden, da dies sonst als Multi behandelt wird!\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "PWS":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen PW-Sharing wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen PW-Sharing mit dem Account '' gesperrt.\r\nDa dies ein Verstoß gegen die Nutzungsbedingungen ist, bleibt der Account gesperrt, bis dieser nach Ablauf der Inaktivenfrist (xDE = 7 Tage, SDE = 7 Tage) gelöscht wird.\r\nBis dahin darf KEIN neuer Account erstellt werden, da dies sonst als Multi behandelt wird!\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "SperrBeleid":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen extremer Beleidigung (= Acclöschung) wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen extremer Beleidigung gesperrt.\r\nAuch in DE gilt die Netiquette, an die man sich bitte halten sollte.\r\nDer Account bleibt gesperrt, bis dieser nach Ablauf der Inaktivenfrist (xDE = 7 Tage, SDE = 7 Tage) gelöscht wird.\r\nBis dahin darf KEIN neuer Account erstellt werden, da dies sonst als Multi behandelt wird!\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "UModeBeleid":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen Beleidigung (= ZwangsUrlaub) wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen Beleidigung für # Tage in den Zwangsurlaub gesetzt.\r\nAuch in DE gilt die Netiquette, an die man sich bitte halten sollte.\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "AWG":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen Accountweitergabe wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen Accountweitergabe gesperrt.\r\nDa dies ein Verstoß gegen die Nutzungsbedingungen ist, bleibt der Account gesperrt, bis dieser nach Ablauf der Inaktivenfrist (xDE = 7 Tage, SDE = 7 Tage) gelöscht wird.\r\nBis dahin darf KEIN neuer Account erstellt werden, da dies sonst als Multi behandelt wird!\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
  case "ReAkt":
   document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen ReAktivierung wurde gesendet\r\n" ;
   document.getElementById("emailtext").value = "Hi,\r\n\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wieder aktiviert.\r\n\r\nMit freundlichen Grüßen\r\nDein DE-Team" ;
   document.getElementById("emailtext").focus();
   break;
 }
 switch(NewMsg) {
 case "autorefresh":
  document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen AutoRefresh wurde gesendet\r\n" ;
  document.getElementById("emailtext").value = "Hi,\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen Nutzung eines Refresh-Skriptes für 3 Tage in den Zwangsurlaub gesetzt.\r\nDas Benutzen von Scripten oder Programmen jeglicher Art, zu Accountverwaltung oder Verbreitung von Accountspezifischen Informationen (Ausnahme: Offizieller DE-Browser), innerhalb oder außerhalb des Spiels ist untersagt. Hierzu zählen auch automatische Abruf- und Refresh-Mechanismen wie sie z.B. der Opera-Browser verwendet.\r\nWir bitten dich dieses Problem zu beheben, ansonsten wird der Account im Wiederholungsfalle endgültig gesperrt.\r\n\Mit freundlichen Grüßen\r\nDein DE-Team";
  document.getElementById("emailtext").focus();
  break;
 }
 switch(NewMsg) {
 case "proxyserver":
  document.getElementById("acckommentar").value = sDateTime + " <?= htmlspecialchars($det_username) ?> - Mail wegen Proxyserver wurde gesendet\r\n" ;
  document.getElementById("emailtext").value = "Hi,\r\ndein Account (<?= htmlspecialchars((string)$user_data["spielername"]) ?>) wurde wegen Nutzung von Proxyservern gesperrt. Da dies ein Verstoß gegen die Nutzungsbedingungen ist, bleiben die Accounts gesperrt, bis diese nach Ablauf der Inaktivenfrist gelöscht werden. Bis dahin darf KEIN neuer Account erstellt werden, da dieser sonst als Multi gesperrt und gelöscht werden würde.\r\n\Mit freundlichen Grüßen\r\nDein DE-Team";
  document.getElementById("emailtext").focus();
  break;
 }
}
...
if (isset($_POST['sendmail'])) {
    csrf_require();
    $acckommentar = req_str('acckommentar');
    $emailtext = req_str('emailtext');
    ...
    echo '<div class="flash flash-ok">eMail an ' . htmlspecialchars((string)$user_data["spielername"]) . ' (' . htmlspecialchars((string)$user_data["reg_mail"]) . ') wurde versandt. (über: ' . htmlspecialchars($det_email) . ') <br> Kommentar wurde dem Useraccount hinzugefügt.</div>';
}

echo '<br><u>eMail an <b>' . htmlspecialchars((string)$user_data["spielername"]) . '</b> (' . htmlspecialchars((string)$user_data["reg_mail"]) . ') senden:</u><br><br>';
